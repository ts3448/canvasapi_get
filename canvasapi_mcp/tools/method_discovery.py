...
class MethodDiscoveryTool:
    ...
    async def execute(self, arguments: dict) -> list[TextContent]:
        ...
        try:
            ...
            result = {
                ...
                "pandas_operations": {
                    ...
                    "operations_count": len(
                        self.pandas_engine.get_available_operations()
                    ),
            ...
            if not include_details:
                result["note"] = (
                    "Use include_details=true for parameter information, or use get_canvas_method_info for specific methods"
                )
            ...
            return [
                TextContent(
                    type="text",
                    text=json.dumps(
                        {"error": "method_discovery_error", "message": str(e)}, indent=2
                    ),
                )
            ]
    ...
    def _get_discovery_target(self, object_type: str, sample_id: int | str | None):
        ...
        try:
            ...
            raise MethodResolutionError(
                f"Failed to get discovery target for {object_type}: {str(e)}"
            )
...
class MethodInfoTool:
        ...
        try:
            ...
            result = {
                ...
                "usage_example": self._generate_usage_example(
                    object_type, method_name, method_info
                ),
            }
            ...
            return [
                TextContent(
                    type="text",
                    text=json.dumps(
                        {"error": "method_info_error", "message": str(e)}, indent=2
                    ),
                )
            ]
    ...
    def _generate_usage_example(
        self, object_type: str, method_name: str, method_info: dict
    ) -> dict:
        """
        ...
        example = {
            "tool": "canvas_query",
            "arguments": {"object_type": object_type, "method": method_name},
        }
        ...
        if "parameters" in method_info and method_info["parameters"]:
            ...
            if example_params:
                example["arguments"]["parameters"] = str(example_params)
...
class PandasOperationsTool:
        ...
        try:
            ...
            if operation_name:
                ...
                result = {
                    ...
                    "usage_example": self._generate_pandas_usage_example(
                        operation_name, all_operations[operation_name]
                    ),
                }
                ...
                if category:
                    filtered_ops = self._filter_operations_by_category(
                        all_operations, category
                    )
                else:
                ...
                result = {
                    ...
                    "operations": (
                        filtered_ops if include_examples else list(filtered_ops.keys())
                    ),
                    "operation_categories": self._get_operation_categories(),
                }

                if include_examples:
                    result["example_sequences"] = (
                        self.pandas_engine.get_operation_examples()
                    )
    ...
    def _generate_pandas_usage_example(
        self, operation_name: str, operation_info: dict
    ) -> dict:
        ...
        base_example = {
            ...
            "arguments": {
                ...
                "pandas_operations": [json.loads(operation_info["example"])],
