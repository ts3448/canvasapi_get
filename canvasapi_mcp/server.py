...
from mcp.types import TextContent, TextResourceContents, ReadResourceResult
...
from .tools.method_discovery import (
    MethodDiscoveryTool,
    MethodInfoTool,
...
class CanvasAPIMCPServer:
    ...
    def __init__(
        ...
        self.canvas_url, self.canvas_token = self._load_configuration(
    ...
    def _load_configuration(
        self, canvas_url: str | None, canvas_token: str | None
    ) -> tuple[str, str]:
        """
        ...
        final_url = (
            canvas_url or os.getenv("CANVAS_URL") or os.getenv("CANVAS_BASE_URL")
        )
        final_token = (
            canvas_token
                    ...
                    return ReadResourceResult(
                        contents=[
                            TextResourceContents(
                                uri=uri, mimeType="application/json", text=str(content)
                            )
                        ]
                    )
        ...
        async def list_tools():
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"DEBUG: method_info_tool type: {type(method_info_def)}"
                    )
                ...
                try:
                    ...
                    logger.info(
                        f"DEBUG: pandas_operations_tool type: {type(pandas_def)}"
                    )
                    logger.info(
                        f"DEBUG: pandas_operations_tool name: {pandas_def.name}"
                    )
        ...
        async def call_tool(name: str, arguments: dict):
            ...
            try:
                ...
                if name == "discover_canvas_methods":
                    ...
                    logger.info(
                        f"DEBUG: pandas_operations_tool returned: {type(result)}"
                    )
                    return result
