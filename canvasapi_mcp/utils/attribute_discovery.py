...
class AttributeDiscovery:
    ...
    def discover_attributes(
        ...
        obj_type = (
            type(obj).__name__.lower() if hasattr(obj, "__dict__") else str(obj).lower()
        )
        ...
        try:
            ...
            if hasattr(obj, "__dict__"):
                ...
                attributes = [
                    attr for attr in obj.__dict__.keys() if not attr.startswith("_")
                ]
        ...
        for attr in attributes:
            ...
            for category, patterns in self.attribute_categories.items():
                for pattern in patterns:
                    if (
                        pattern == attr_lower
                        or pattern in attr_lower
                        or attr_lower.endswith(pattern)
                        or attr_lower.startswith(pattern)
                    ):
    ...
    def _discover_relationships(self, object_type: str, attributes: list[str]) -> dict:
        ...
        for attr in attributes:
            ...
            if any(
                pattern in attr_lower for pattern in ["parent_", "root_", "account_"]
            ):
                relationships["parent_objects"].append(attr)

            # Child reference indicators (arrays/lists)
            elif any(
                pattern in attr_lower for pattern in ["_ids", "_list", "children"]
            ):
                relationships["child_references"].append(attr)

            # Many-to-many indicators
            elif any(
                pattern in attr_lower for pattern in ["tags", "groups", "members"]
            ):
    ...
    def _calculate_importance_scores(
        self, attributes: list[str], object_type: str
    ) -> dict[str, float]:
        ...
        for attr in attributes:
            ...
            if any(
                ...
                for pattern in ["state", "status", "published", "active"]
            ):
                score += 1.0
            ...
            if any(
                pattern in attr_lower for pattern in ["description", "body", "syllabus"]
            ):
                score += 0.3
    ...
    def get_core_attributes(
        self, object_type: str, max_attributes: int = 20
    ) -> list[str]:
        """
    ...
    def discover_object_schema(
        self, objects: list[CanvasObject], sample_size: int = 100
    ) -> dict:
        """
        ...
        attribute_stats = defaultdict(
            lambda: {"count": 0, "types": set(), "samples": []}
        )
        ...
        for attr_name, stats in attribute_stats.items():
            schema[attr_name] = {
                ...
                "primary_type": max(
                    stats["types"], key=lambda t: str(stats["types"]).count(t)
                ),
        ...
        statistics = {
            ...
            "most_common_attributes": sorted(
                schema.keys(), key=lambda attr: schema[attr]["frequency"], reverse=True
            )[:10],
            "coverage_distribution": {
                "high_coverage": len(
                    [a for a in schema.values() if a["coverage"] > 0.9]
                ),
                "medium_coverage": len(
                    [a for a in schema.values() if 0.5 < a["coverage"] <= 0.9]
                ),
                "low_coverage": len(
                    [a for a in schema.values() if a["coverage"] <= 0.5]
                ),
            },
        }

        return {"schema": schema, "statistics": statistics, "object_type": object_type}

    def suggest_dataframe_columns(
        self, schema_info: dict, max_columns: int = 30, min_coverage: float = 0.1
    ) -> list[str]:
        """
        ...
        for attr_name, attr_info in schema.items():
            if attr_info["coverage"] >= min_coverage:
                ...
                if any(
                    pattern in attr_name.lower() for pattern in ["id", "name", "title"]
                ):
                    score += 0.3
        ...
        for obj in sample:
            for attr_name, attr_value in obj.__dict__.items():
                ...
                # Check for lists of Canvas objects
                    ...
                    and attr_value
                    and isinstance(attr_value[0], CanvasObject)
                ):
