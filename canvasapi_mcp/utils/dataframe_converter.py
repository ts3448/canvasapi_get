...
class DataFrameConverter:
    ...
    def __init__(
        self, batch_size: int = 1000, max_depth: int = 3, memory_limit_mb: int = 500
    ):
        """
    ...
    def _convert_object_list(
        ...
        for i in range(0, len(objects), self.batch_size):
            ...
            batch_rows = [
                self._flatten_canvas_object(obj, flatten_nested) for obj in batch
            ]
    ...
    def _flatten_canvas_object(
        self, obj: CanvasObject, flatten_nested: bool = True, depth: int = 0
    ) -> dict:
        """
        ...
        for attr_name, attr_value in obj.__dict__.items():
            ...
            if (
                isinstance(attr_value, CanvasObject)
                and flatten_nested
                and depth < self.max_depth
            ):
                nested_flattened = self._flatten_canvas_object(
                    attr_value, flatten_nested, depth + 1
                )
            ...
            # Handle lists of Canvas objects
                ...
                if flatten_nested and depth < self.max_depth:
                    ...
                    if len(attr_value) > 0:
                        ...
                        first_item = self._flatten_canvas_object(
                            attr_value[0], False, depth + 1
                        )
            ...
            # Handle regular lists
                ...
                if len(attr_value) <= 10:  # Small lists as comma-separated strings
                    ...
                    flattened[f"{attr_name}.count"] = len(attr_value)
                    flattened[f"{attr_name}.sample"] = ", ".join(
                        str(v) for v in attr_value[:3]
                    )
    ...
    def _optimize_dataframe_schema(
        self, df: pd.DataFrame, object_type: str
    ) -> pd.DataFrame:
        """
        ...
        for column in df.columns:
            try:
                ...
                if column_info["dtype"] != "object":
                    optimized_df[column] = self._convert_column_dtype(
                        df[column], column_info
                    )
    ...
    def _analyze_column(
        ...
        if any(
            id_pattern in column_name.lower() for id_pattern in ["id", "_id", "uuid"]
        ):
            # Determine if numeric or string ID
            if non_null_series.dtype in ["int64", "int32"] or all(
                str(v).isdigit() for v in non_null_series.head(100)
            ):
        ...
        if any(
            date_pattern in column_name.lower()
            for date_pattern in ["date", "time", "created", "updated", "due"]
        ):
        ...
        for column, info in schema_info.items():
            if column in df.columns and info["dtype"] != "object":
                try:
                    ...
                    logger.warning(
                        f"Cached schema application failed for {column}: {str(e)}"
                    )
...
def convert_to_dataframe(
    data: CanvasObject | list[CanvasObject] | PaginatedList, **kwargs
) -> pd.DataFrame:
    """
