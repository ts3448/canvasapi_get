...
class PandasEngine:
    ...
    def apply_operations(self, dataframe: pd.DataFrame, operations: Sequence[dict]) -> pd.DataFrame:
        """
        ...
        try:
            for i, operation in enumerate(operations):
                ...
                if operation_name == 'groupby':
                    ...
                    if i + 1 >= len(operations) or operations[i + 1].get('operation') != 'agg':
                        raise PandasOperationError(
                ...
                operation_results.append({
                    ...
                    'rows_before': len(dataframe) if i == 0 else operation_results[i-1].get('rows_after', 0),
                    'rows_after': len(result) if hasattr(result, '__len__') and hasattr(result, 'columns') else 'N/A',
                    'columns': len(result.columns) if hasattr(result, 'columns') else 'N/A',
    ...
    def _execute_operation(self, dataframe: pd.DataFrame, operation: dict) -> pd.DataFrame:
        """
        ...
        try:
            if operation_name == 'query':
                ...
                if hasattr(dataframe, 'agg') and hasattr(dataframe, 'obj'):
                    ...
                    raise PandasOperationError("agg operation can only be used after groupby")
                ...
                if len(dataframe.columns) == 1:
                    ...
                    raise PandasOperationError("value_counts requires a single column DataFrame")
    ...
    def get_operation_examples(self) -> dict[str, list[dict]]:
        ...
        return {
            ...
            "data_cleaning": [
                {"operation": "drop_duplicates", "subset": ["user_id"], "keep": "first"},
