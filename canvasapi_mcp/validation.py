...
class ParameterValidator:
    ...
    def validate_parameter_type(
        ...
        if isinstance(expected_type, str):
            expected_type = self.PARAMETER_TYPES.get(expected_type.lower(), expected_type)
        ...
        if expected_type == bool:
            ...
            try:
                ...
                raise ValidationError(param_name, f"must be an integer, got {type(value).__name__}")
        
        elif expected_type == float:
            try:
                return float(value)
            except (ValueError, TypeError):
                raise ValidationError(param_name, f"must be a float, got {type(value).__name__}")
            ...
            if isinstance(value, str):
                ...
                raise ValidationError(param_name, f"must be a date/datetime or string, got {type(value).__name__}")
        
        elif expected_type in ((list, tuple), list, tuple):
            if not is_multivalued(value):
                raise ValidationError(param_name, f"must be a list or array, got {type(value).__name__}")
            return list(value) if not isinstance(value, list) else value
        
        elif expected_type == dict:
            if not isinstance(value, dict):
                raise ValidationError(param_name, f"must be an object/dict, got {type(value).__name__}")
            return value
    ...
    def convert_canvas_object_to_dict(self, obj: CanvasObject) -> Dict[str, Any]:
        ...
        if not isinstance(obj, CanvasObject):
            raise ValidationError("obj", f"must be a CanvasObject, got {type(obj).__name__}")
        ...
        for attr_name, attr_value in obj.__dict__.items():
            if not attr_name.startswith('_') and attr_name != 'attributes':
                ...
                if isinstance(attr_value, CanvasObject):
                    ...
                    result[attr_name] = [
                        self.convert_canvas_object_to_dict(item) if isinstance(item, CanvasObject) else item
    ...
    def extract_canvas_ids(self, obj: CanvasObject) -> Dict[str, Union[int, str]]:
        ...
        id_patterns = [
            'id', 'sis_user_id', 'sis_course_id', 'sis_account_id', 'sis_section_id',
            'integration_id', 'lti_user_id', 'uuid', 'course_code', 'account_id',
            'user_id', 'group_id', 'section_id', 'enrollment_id'
        ]
    ...
    def validate_canvas_method_parameters(
        ...
        try:
            ...
            for param_name, param_info in sig.parameters.items():
                if (param_info.default == inspect.Parameter.empty and 
                    param_name not in ['self', 'args', 'kwargs'] and
